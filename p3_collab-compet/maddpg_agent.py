...
class Group():
    ...
    def checkpoint(self):
        return [{ 'actor': agent.actor_local.state_dict(),
                 'critic': agent.critic_local.state_dict()} for agent in self.agents]
        
class Agent():
    """Interacts with and learns from the environment."""
    
    ##$$ additional input here compared to ddpg: sharing memory
    def __init__(self, memory, state_size, action_size, random_seed): 
        ...
        # Replay memory
        # self.memory = ReplayBuffer(action_size, BUFFER_SIZE, BATCH_SIZE, random_seed)
        self.memory = memory ##$$ use the memory shared by the group
